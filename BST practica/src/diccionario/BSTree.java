...
public class BSTree<E extends Comparable<E>> {
	...
	public LinkedList<E> productoPorTerminar(E producto){ 
		return productoPorTerminar (this.root, producto);
	}
	
	private LinkedList<E> productoPorTerminar(Node node, E producto){
		Node actual= search(producto, node);
		LinkedList<E> prod= new LinkedList<E> ();
		while (actual!=null) {
			if (producto.compareTo(actual.data) > 0)
				prod.add(actual.data);
			actual = actual.left;
		}
		return prod;
	}
	...
	public void amplitud(){
		this.amplitud(this.root);
	}
	
	private void amplitud(Node n) { 
		LinkedQueue <Node> cola = new LinkedQueue <Node>();
		Node actual= n;
		try {
			if (actual !=null){
				cola.enqueue(actual);
